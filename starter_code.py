...
def train(epoch, model, trainloader, optimizer, criterion, CONFIG):
    ...
    progress_bar = tqdm(
        trainloader, desc=f"Epoch {epoch + 1}/{CONFIG['epochs']} [Train]", leave=False
    )
...
def main():
    ...
    ############################################################################
    import src.cv.evaluate.cifar100 as cifar100
    import src.cv.evaluate.ood as ood

    # --- Evaluation on Clean CIFAR-100 Test Set ---
    predictions, clean_accuracy = cifar100.evaluate_cifar100_test(
        model, testloader, CONFIG["device"]
    )
    print(f"Clean CIFAR-100 Test Accuracy: {clean_accuracy:.2f}%")

    # --- Evaluation on OOD ---
    all_predictions = ood.evaluate_ood_test(model, CONFIG)

    # --- Create Submission File (OOD) ---
    submission_df_ood = ood.create_ood_df(all_predictions)
